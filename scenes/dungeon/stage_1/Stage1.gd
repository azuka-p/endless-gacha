...
func _on_Attack_pressed():
	...
	$Player/HealthBar.value = stepify(
		float(StageData.team_stats[current].hp) / StageData.max_hp[current] * 100, 0.01
	)
...
func character_died():
	...
	StageData.team_stats.remove(current)
	StageData.max_hp.remove(current)
